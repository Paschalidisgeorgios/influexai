...
function ProduktWerbungPageInner() {
  ...
  useEffect(() => {
    ...
    if (!title && !hook && !description && !script) return;
    if (script) {
      setProduct(script);
    } else {
      const parts: string[] = [];
      ...
      if (hook) parts.push(`Hook (3s): ${hook}`);
              ...
              {PLATFORMS.map((p) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "0.7rem", color: "#505055" }}>
                      {p.format}
                    </div>
                  </div>
                  {platform === p.id && (
...
function ResultCard({
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontSize: "0.82rem", fontWeight: 700, color: "#F0EFE8" }}>
          {title}
        </div>
        <button
          onClick={onCopy}
