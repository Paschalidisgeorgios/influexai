...
import { QUALITY_CHECK_IDS } from "@/data/landingAgentDemos";
...
export function QualityIntelligenceSection() {
  ...
  return (
    <section
      ...
      aria-label={t("qualitySection.kicker")}
    >
      <div className="mx-auto max-w-[1160px]">
        <SpringReveal>
          <span className="kicker mb-2 block">{t("qualitySection.kicker")}</span>
          <h2 className="landing-heading mb-3 text-[clamp(1.75rem,3.5vw,3rem)]">
            {t("qualitySection.headline1")}
            <br />
            <span className="acid-highlight">{t("qualitySection.headline2")}</span>
          </h2>
          <p
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3 md:gap-4">
          {QUALITY_CHECK_IDS.map((areaId, i) => {
            const prefix = `quality.${areaId}`;
            const items = t.raw(`${prefix}.items`) as string[];

            return (
              <SpringReveal key={areaId} delay={i * 0.08}>
                <article
