  ...
  return (
    <article
      ...
      className={`landing-v2-proof-card landing-v2-ivory-stage flex h-full flex-col p-4 md:p-5 ${
        enableCinematic && proofKey ? "landing-v2-panel-3d" : ""
      }`}
      data-lv2-stagger
    >
...
export function LandingV2Proof() {
  ...
  useProof3DScene(sectionRef, sceneRef, enableCinematicScroll);
        ...
        >
          <div
            className={`landing-v2-proof-scene__rig landing-v2-proof-scene__rig--quad ${
              enableCinematicScroll ? "landing-v2-scene-3d__rig" : ""
            }`}
          >
            {copy.cards.map((card) => (
              <OutputCard
                key={card.id}
                card={card}
                enableCinematic={enableCinematicScroll}
                proofKey={
