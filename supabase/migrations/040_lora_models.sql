-- LoRA models table for AI creator training
...
AND tablename = 'lora_models'
AND policyname = 'Users can manage own loras'
) THEN
...
ON public.lora_models (user_id, created_at DESC);
...
AND policyname = 'Users upload own lora training files'
) THEN
...
WHERE schemaname = 'storage'
AND tablename = 'objects'
AND policyname = 'Users read own lora training files'
) THEN
...
WHERE schemaname = 'storage'
AND tablename = 'objects'
AND policyname = 'Users delete own lora training files'
) THEN
